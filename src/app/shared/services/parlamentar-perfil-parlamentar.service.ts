...
export class ParlamentarService {
  private url = environment.perfilUrl + '/parlamentares';
  ...
  getInfoById(id: string): Observable<Parlamentar> {
    return this.http
      .get<Parlamentar>(this.url + '/' + id + '/info')
      .pipe(map(parlamentar => {
        return new Parlamentar(parlamentar);
      }));
  }
  ...
  getInvestimentoById(id: string): Observable<ParlamentarInvestimento> {
    return this.http
      .get<ParlamentarInvestimento>(environment.perfilUrl + 'investimento/parlamentar/' + id);
  }

  getInvestimentoPartido(idPartido: number, uf: string, esfera: string): Observable<PartidoInvestimento> {
    return this.http
      .get<PartidoInvestimento>(environment.perfilUrl + 'investimento/partido/' + idPartido + '/' + uf + '/' + esfera);
  }
}
